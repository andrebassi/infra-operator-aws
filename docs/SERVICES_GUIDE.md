...
### 1. Use Deletion Policies Wisely

- `Retain`: For production databases, S3 buckets with important data
- `Delete`: For development/testing resources
- `Orphan`: When transitioning management away from operator
...
### 3. Use K8s Secrets for Sensitive Data

Instead of:
```yaml
spec:
  masterUsername: admin
  masterUserPassword: "MyPassword123"  # Bad!
```
...
- RDS instances
- Secrets Manager
- SQS/SNS (when handling sensitive data)

### 5. Multi-AZ for Production

For high availability:
- RDS: `multiAZ: true`
- ElastiCache: Multiple nodes across AZs
...
```bash
# Watch all resources
kubectl get vpcs,subnets,s3buckets,dynamodbtables -o wide
