...
O **infra-operator** segue os princípios de **Hexagonal Architecture** (também conhecida como Ports and Adapters), que é mais adequada para Go do que Clean Architecture tradicional.

### Por que Hexagonal Architecture?

Baseado em pesquisa sobre melhores práticas ([Clean Architecture in Go](https://pkritiotis.io/clean-architecture-in-golang/), [Kubernetes Operator Best Practices](https://sdk.operatorframework.io/docs/building-operators/golang/tutorial/)):

1. **Simplicidade**: Menos camadas que Clean Architecture tradicional
...
4. **Idempotência**: Controllers Kubernetes precisam ser idempotentes - arquitetura ajuda nisso
...
**Responsabilidade**: Define contratos (interfaces) que adaptadores devem implementar.
...
// S3Repository define WHAT we need, not HOW
...
// S3UseCase define business operations
...
**Responsabilidade**: Implementa as interfaces usando tecnologias específicas (AWS SDK v2).
...
- ✅ Converte entre domain types e AWS types
...
**Responsabilidade**: Reconcile loop, watch CRDs, update status.
...
**Características**:
- ✅ Mínima lógica - só orchestração Kubernetes
- ✅ Depende de use cases (não de adapters diretamente)
...
│   │   ├── s3_usecase.go           # Interface para casos de uso
...
│   ├── mapper/                     # Conversão CR ↔ Domain
│   │   ├── s3_mapper.go
│   │   └── lambda_mapper.go
│   └── clients/                    # Factories para clients
│       └── aws_client.go
│
└── cmd/
...
```
1. User aplica S3Bucket CR
   │
   ▼
2. Kubernetes API Server persiste CR
   │
   ▼
...
### 2. Testes de Use Case (com Mock Repository)
...
### 3. Testes de Controller (com Mock UseCase)
...
### ✅ Para Kubernetes Operators

1. **Idempotência**: Use cases garantem operações idempotentes
2. **Reconcile Simples**: Controller só orquestra, não tem lógica
3. **Status Reporting**: Fácil atualizar status baseado em domain state

### ✅ Para Testes

1. **Domain**: Testes puros, sem mocks
...
2. **Baixo Acoplamento**: Mudanças em AWS não afetam domain
3. **Alta Coesão**: Código relacionado fica junto

### ✅ Para Extensibilidade

1. **Múltiplos Clouds**: Trocar adapter AWS por GCP/Azure
2. **Múltiplos Backends**: Adicionar adapter para Terraform/Pulumi
3. **Evolução**: Mudar AWS SDK v2 → v3 sem afetar domain
